...
export default function CookieConsentContent({ 
  ...
  const [openStreetMapConsent, setOpenStreetMapConsent] = useState<boolean | null>(initialOpenStreetMap);
              ...
              <div className="flex-1">
                ...
                {openStreetMapConsent !== true && (
                  <p className="text-xs text-orange-600 font-medium mt-1">
                    ⚠️ Google Maps benötigt OpenStreetMap, da die Gartenumrisse aus OSM-Daten stammen.
                  </p>
                )}
              </div>
              <div className="ml-4">
                <label className={`relative inline-flex items-center ${
                  openStreetMapConsent === true ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
                }`}>
                  <input
                    type="checkbox"
