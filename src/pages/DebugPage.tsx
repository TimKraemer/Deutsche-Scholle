...
import { searchGardenByNumber, loadAllGardens } from '../utils/osm';
import { osmWayToGarden } from '../utils/osm';
...
type SortColumn = 'number' | 'parcel' | 'dbSize' | 'osmSize' | 'difference' | 'percentageDiff' | 'status';
  ...
  useEffect(() => {
    const loadComparisons = async () => {
      ...
      for (const mockGarden of mockGardens) {
        ...
        const osmWay = allOsmGardens.find(
          (osm) => osm.tags?.ref === mockGarden.number
        ) || null;
        ...
        <div className="container mx-auto max-w-6xl">
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
            ...
            <p className="text-yellow-700">
              Bitte aktivieren Sie OpenStreetMap in den Cookie-Einstellungen, um die Debug-Ansicht zu verwenden.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto max-w-7xl">
        <div className="mb-6">
          ...
          <p className="text-scholle-text-light">
            Vergleich der Größenangaben aus der Datenbank mit den aus OSM-Geometrie berechneten Werten
          </p>
        </div>
          ...
          <div className="bg-scholle-bg-container rounded-lg border border-scholle-border shadow-sm overflow-hidden">
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-scholle-border">
                  {sortedComparisons.map((comp) => {
                    const hasLargeDiff = comp.percentageDiff !== null && Math.abs(comp.percentageDiff) > 10;
                    const diffColor = comp.difference === null 
            ...
            <div className="px-4 py-3 bg-scholle-bg-light border-t border-scholle-border">
              <div className="flex items-center justify-between text-sm text-scholle-text-light">
                <div>
                  Gesamt: {comparisons.length} Gärten
                </div>
                <div>
                  Mit OSM-Daten: {comparisons.filter(c => c.hasOsmData).length} | 
                  Ohne OSM-Daten: {comparisons.filter(c => !c.hasOsmData).length}
                </div>
